  ...
  try {
    ...
    if (!turnstile.success) {
      ...
      return NextResponse.json(
        { error: isNotConfigured ? "Капча не налаштована на сервері. Додайте TURNSTILE_SECRET_KEY в .env" : "Captcha verification failed" },
    ...
    const sent = await sendEmail({
      ...
      textContent: `Ваш код: ${code}. Діє 5 хв.`,
      htmlContent: `<p>Ваш код входу: <strong>${code}</strong></p><p>Діє 5 хвилин.</p>`,
    });
    if (!sent.ok) {
      return NextResponse.json({ error: "Не вдалося надіслати код. Спробуйте пізніше." }, { status: 500 });
    }
